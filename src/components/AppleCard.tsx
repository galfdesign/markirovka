...
const AppleCard: React.FC<AppleCardProps> = ({
  ...
  const calculateResults = () => {
    ...
    // Определение мощности на метр в зависимости от шага укладки
    let wattsPerMeter;
    ...
    // Коэффициент для температурной дельты
    let deltaCoef = 1;
  ...
  return (
    <div className="apple-card" onClick={onEdit}>
      <div className="apple-card-header">
        <div className="apple-card-title">Петля №{number}</div>
      ...
      <div className="apple-card-content">
        ...
        <div className="apple-card-field">
          <label>Расход</label>
          <div className="apple-card-value">{flowRate !== undefined ? flowRate.toFixed(2) + ' л/мин' : (collector || '-')}</div>
        </div>
